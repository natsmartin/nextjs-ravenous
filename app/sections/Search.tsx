...
export default function Search(): React.JSX.Element {
  ...
  const handleSort = (e: React.MouseEvent) => {
    ...
    setValueState((prev) => ({ ...prev, sortby: value }));
    handleSearch();
  };

  const handleSearch = () => {
    if (!valueState.term || !valueState.location) {
      setModal("block");
    }
  };
  ...
  return (
    <>
      <div
        className={`${modal} fixed bg-[#0006] z-20 top-0 left-1/2
      ...
      <div className="bg-cyan-900 z-10 relative flex flex-col justify-center">
        <Image
          ...
          fill
          src={"/search-background.webp"}
          alt={"Ravenous background image"}
          priority={true}
        />
        <form
          action={formAction}
        ...
        >
          ...
          <input
            id="hidden-input-sort"
            className="hidden"
            value={valueState.sortby}
          ...
          <div className="flex flex-col justify-center items-center md:flex md:flex-row w-auto md:w-[950px]">
            <Input
              id="term-input"
              placeholder="What food craves you?"
              name="term"
              value={valueState.term}
              onChange={handleInputChange}
            />
            <Input
              id="location-input"
          ...
          <div className="flex justify-center m-2">
            <button
              type="submit"
              className="rounded-lg bg-cyan-700 text-xs text-white font-semibold md:font-bold px-8 py-2 
